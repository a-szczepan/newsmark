...
export const EditAnnotation: React.FC<EditAnnotationProps> = ({
  ...
  const [addAnnotation, { data: addded, isSuccess: addedSuccess }] = useAddAnnotationMutation({})
  ...
  const [selectedColor, setSelectedColor] = useState(data ? data.colorValue : 'green')
  ...
  return (
    <form
      ...
      {addNote ? (
        <Textarea label="Note" reference={noteRef} {...(data && { value: data.noteValue })} />
      ) : (
        <Button
...
export const ReadAnnotation: React.FC<ReadAnnotationProps> = ({
  ...
  return (
    <div>
      ...
      {data.noteValue && (
        <Textarea name="note" label="Note" rows={5} readOnly value={data.noteValue} />
      )}
      <div className={styles.iconButtonsGroup}>
        ...
        <IconButton
          ...
          buttonAction={() => {
            deleteAnnotation(annotationId).then(() => handleAnnotationDeleteComplete())
          }}
        />
      </div>
    </div>
